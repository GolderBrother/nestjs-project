  ...
  async startSpider() {
    ...
    for (let i = 1; i <= totalPage; i++) {
      ...
      const jobList = await page.$eval('.job-list-box', (el) => {
        return Array.from(el.querySelectorAll('.job-card-wrapper')).map(
          (item) => {
            // 具体的信息都是从 dom 去拿的：
      ...
      for (const job of allJobs) {
        ...
        try {
          ...
          jobEntity.area = job.job.area;
          jobEntity.name = job.job.name;
          jobEntity.salary = job.job.salary;
          jobEntity.link = job.link;
          jobEntity.company = job.company.name;
          jobEntity.desc = job.desc;
          console.log('job', job);
          this.entityManager.save(jobEntity);
        } catch (error) {
